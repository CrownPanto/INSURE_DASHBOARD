...
def show_page(session, selected_ym):
    ...
    if len(_cmp_rows) == 2:
        _r1, _r2 = _cmp_rows[0], _cmp_rows[1]
        _pricier = _r1 if _r1["ADJUSTED_PREMIUM_MONTHLY"] > _r2["ADJUSTED_PREMIUM_MONTHLY"] else _r2
        _cheaper = _r2 if _pricier is _r1 else _r1

        if not is_demo:
            # ── Cortex LLM 프롬프트 (Few-shot 포함) ──
            _prompt = (
            ...
            try:
                ...
                st.markdown(f"""
                    <div style="background:linear-gradient(135deg,rgba(16,185,129,0.08),rgba(16,185,129,0.02));
                                border:1.5px solid rgba(16,185,129,0.3);border-left:4px solid #10b981;
                                border-radius:12px;padding:16px 22px;margin-top:16px;">
                        <div style="font-size:11px;color:#10b981;font-weight:700;
                                    letter-spacing:1px;text-transform:uppercase;margin-bottom:8px;">
                            ✨ AI 보험료 분석 · Cortex LLM
                        </div>
                ...
                # Cortex 실패 시 정적 폴백
                _diff_w = abs(_r1["ADJUSTED_PREMIUM_MONTHLY"] - _r2["ADJUSTED_PREMIUM_MONTHLY"])
                _base_ratio = _pricier["AVG_BASE_PREMIUM"] / max(_cheaper["AVG_BASE_PREMIUM"], 1)
                _risk_diff = abs(_r1["COMPOSITE_RISK_SCORE"] - _r2["COMPOSITE_RISK_SCORE"])
                if _base_ratio > 1.2:
                    _reason = f"자산 보호가액이 {_base_ratio:.1f}배 높기 때문"
                elif _risk_diff > 3:
                    _reason = f"위험도 점수가 {_risk_diff:.1f}점 높기 때문"
                else:
                    _reason = "복합적인 위험도 및 자산 요인"
                st.info(
                    f"💡 {_pricier['GU_NAME']}가 {_cheaper['GU_NAME']}보다 "
                    f"월 ₩{_diff_w:,.0f} 더 비싼 주된 이유 — {_reason}입니다."
                )
        else:
            # 데모 모드 폴백
            _diff_w = abs(_r1["ADJUSTED_PREMIUM_MONTHLY"] - _r2["ADJUSTED_PREMIUM_MONTHLY"])
            st.info(
                f"💡 서초구는 아파트 평균 매매가가 서울 평균의 2배 수준으로 "
                f"자산 보호가액이 높아, 위험도가 유사한 영등포구보다 "
                f"월 ₩{_diff_w:,.0f} 더 비쌉니다."
            )
