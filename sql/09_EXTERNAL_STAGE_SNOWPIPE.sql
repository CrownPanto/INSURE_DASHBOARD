...
CREATE OR REPLACE TASK TASK_REFRESH_INTERMEDIATE
    ...
    COMMENT = 'Intermediate 레이어 재계산 (세그먼트 + 리스크)'
AS
BEGIN
    -- 세그먼트 분류 재실행
    ...
    -- (실제로는 05_DBT_INTERMEDIATE.sql의 전체 로직 재실행)

    -- 리스크 스코어 재계산
    CREATE OR REPLACE TABLE INSURE_DB.INTERMEDIATE.INT_DISTRICT_RISK_SCORE AS
    SELECT * FROM INSURE_DB.INTERMEDIATE.INT_DISTRICT_RISK_SCORE LIMIT 0;
END;
...
CREATE OR REPLACE TASK TASK_REFRESH_MART
    ...
    COMMENT = 'Mart 레이어 재계산 (보험 설계)'
AS
BEGIN
    CREATE OR REPLACE TABLE INSURE_DB.MART.MART_INSURANCE_DESIGN AS
    SELECT * FROM INSURE_DB.MART.MART_INSURANCE_DESIGN LIMIT 0;
    CREATE OR REPLACE TABLE INSURE_DB.MART.MART_DISTRICT_INSURANCE_SUMMARY AS
    SELECT * FROM INSURE_DB.MART.MART_DISTRICT_INSURANCE_SUMMARY LIMIT 0;
END;
