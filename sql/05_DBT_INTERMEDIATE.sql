...
-- ============================================================
-- MERGED FROM: 12_FIX_GU_COVERAGE_AND_RISK.sql
-- Purpose: 25개 구 전체 커버리지 리스크 스코어링 보정
-- ============================================================
...
-- ★ v2.4 FIX: 시드 데이터(2021~2024)만 존재하므로 2025년 리스크 데이터 보충
-- 최신 연도 데이터를 2025년으로 복제 (이미 존재하면 SKIP)
...
WHERE YEAR = (SELECT MAX(YEAR) FROM INT_DISTRICT_RISK_SCORE)
  AND NOT EXISTS (
    ...
    WHERE r2.YEAR = 2025
      AND TRIM(r2.DISTRICT_NAME) = TRIM(INT_DISTRICT_RISK_SCORE.DISTRICT_NAME)
  );

SELECT 'INTERMEDIATE layer complete with v1.1 coverage fix + v2.4 year gap fix' AS status;
